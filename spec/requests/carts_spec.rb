...
RSpec.describe "/carts", type: :request do
  ...
  describe "POST /add_item" do
    ...
    context 'success cases' do
      it "substitutes the quantity of the existing item in the cart directly" do
        cart = create(:cart)
        product = create(:product, price: 10.0)
        cart_item = create(:cart_item, cart: cart, product: product, quantity: 1)
        cart.update_total!
        
        post endpoint, params: { cart_id: cart.id, product_id: product.id, quantity: 5 }, as: :json
        ...
        response_body = response.parsed_body
        expect(response_body['id']).to eq(cart.id)
        expect(response_body['total_price']).to eq("50.0")

        product_data = response_body['products'].first
        expect(product_data['id']).to eq(product.id)
        expect(product_data['name']).to eq(product.name)
        expect(product_data['quantity']).to eq(5)
        expect(product_data['unit_price']).to eq("10.0")
        expect(product_data['total_price']).to eq("50.0")
      end
    end
  end
end
